...
```{r}
spline <- smooth.spline(x, y, cv = TRUE, all.knots = FALSE)
plot(x,y)
...
df <- spline$df
print(paste0("Number of lambda = ",spline$lambda))
print(paste0("Number of df = ",spline$df))
print(paste0("Number of knots = ",spline$fit$nk-2))
```
##Part B
```{r}
#
...
x2<-bikes$temp
y2<-bikes$cnt.5000
log_spl <-logistic.IRWLS.splines(x = x2, y = y2, df = 6, plts = TRUE)
```

```{r}
temp_05 = bikes$temp[log_spl$fitted.values>0.5]
```
