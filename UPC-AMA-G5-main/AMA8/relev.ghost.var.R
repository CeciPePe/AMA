...
relev.ghost.var <- function(model, newdata=model$call$data, y.ts=NULL, 
                           ...
                           permut.gam.vars =NULL, isRanger = FALSE,...){
  ...
  if(isRanger){
    y.hat.ts <- as.numeric( predict(model,data = newdata)$predictions )
  }else{
  ...
  for (j in (1:p)){
    ...
    if(isRanger){
      y.hat.ts.j <- as.numeric( predict(model,data = yX.ts.aux)$predictions )
